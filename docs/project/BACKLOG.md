...
- **Известно:** Физический artifact 0.2.9 от 2026-08-13 завершил pairing и рабочий tunnel, но остался в `connecting`: Mac Syncthing не содержал paired Windows device. Root cause подтверждён read-only диагностикой и кодом — device configuration выполнялась только позднее в workspace preflight. В активной ветке `codex/fix-syncthing-bootstrap` @ `9a76d76` connection runtime сначала идемпотентно применяет точную Syncthing-пару через существующий loopback relay/typed RPC и сохраняет текущие managed folders. Автоматические race-тесты не доказывают результат на двух ОС.
- **Критерий завершения:** После fresh pairing оба UI достигают `connected`, Syncthing содержит только exact paired devices без dummy folder, а restart восстанавливает соединение без нового pairing. Затем изменения Mac source достигают WSL до Docker execution; conflict/error виден; поддерживаемый TCP port доступен на том же свободном Mac localhost port; чужой local listener не завершается. Кандидат поверх воспроизведённой повреждённой identity восстанавливается без потери workspaces и завершает pairing/sync.
- **Проверка:** Новый точный Mac/Windows artifact: fresh pairing, Syncthing device/connections snapshot и restart; затем малые и крупные файлы, rename/delete, rapid edits, Compose bind mount, port conflict и removal container publication. Отдельно — update Mac-кандидата поверх повреждённой identity, повторное pairing и базовая Docker-команда.
...
- **Критерий завершения:** После Pause/Finish work отсутствуют app-owned tunnel, Syncthing, SSH forwarding, presence SSH child, watchdog и managed workload processes; фиксированные ports освобождены; CPU idle; память/handles/connections не растут без границы в длительной сессии. В Connected один session-owned presence child переживает таймауты успешных heartbeat-запросов и не пересоздаётся после каждого ответа. Ошибка или timeout focus восстанавливает только exact owned UI одной bounded попыткой; неуспешный recovery не затрагивает foreign/newer process. Finish work одновременно с focus/Stop не оставляет UI child и не допускает поздний relaunch.
- **Проверка:** Process/port snapshots до запуска, connected, paused и finished; длительная idle/active сессия с периодическими samples. На Mac отдельно подтвердить стабильный `connected`, повторные успешные heartbeat и один persistent presence SSH child, а после Pause/Finish work — его завершение. На Windows отдельно вызвать успешный и неуспешный exact-UI recovery и гонку Finish work с focus/Stop, сверяя process ownership до и после.
