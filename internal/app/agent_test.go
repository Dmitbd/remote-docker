...
func TestAgentHandlerRoutesAllControlOperations(t *testing.T) {
	...
	methods := []localapi.Method{
		...
		localapi.MethodPairStatus,
		localapi.MethodPairApprove,
		localapi.MethodPairReject,
		localapi.MethodPairCancel,
...
func TestAgentCLIControlCommandsAndJSONOutput(t *testing.T) {
	tests := []struct {
		...
		{name: "pair status", args: []string{"pair", "status", "session"}, method: localapi.MethodPairStatus},
...
func (c *trustRevokedBindingController) retryTrustRevokedObserver(ctx context.Context, deviceID, generation string) error {
	c.retryCalls++
	c.retryDeviceID = deviceID
	c.retryGeneration = generation
	return c.observer(ctx, deviceID, generation)
}
