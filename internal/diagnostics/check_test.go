...
func TestRunnerChecksEveryDependencyInStableOrder(t *testing.T) {
	...
	if results[4].OK || results[4].Reason == "" || results[4].Reason == "dockerd replied Authorization: Bearer never-show-this-token" {
		t.Fatalf("Docker result = %#v, want a redacted failure", results[4])
	}
}
...
func TestRunnerReportsUnavailableCheckWithStableSafeReason(t *testing.T) {
	...
	for index, result := range results {
		if result.Name != orderedCheckNames[index] || result.OK || result.Reason != ErrCheckUnavailable.Error() {
