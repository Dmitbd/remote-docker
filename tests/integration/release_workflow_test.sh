...
grep -F -- '-Version 0.2.6' "${ci_workflow}" >/dev/null || fail "Windows CI preview version must use the desktop application generation"
grep -F 'Remote-Docker-0.2.6-x64-Setup.exe' "${ci_workflow}" >/dev/null || fail "Windows CI must verify exactly one Setup EXE"
